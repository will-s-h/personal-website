                ...
                <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                    <Link href="/" className="flex items-center text-black dark:text-white" style={{textDecoration: 'none'}}>
                        <Image
                            ...
                            alt="profile"
                        />
                        <span className="self-center text-xl font-light whitespace-nowrap font-sans">William Huang</span>
                    </Link>
                            ...
                            <div id="dropdownAvatar" className={`absolute right-0 top-11 z-10 ${isNavOpen ? '' : 'hidden'} bg-white divide-y divide-gray-100 rounded-lg shadow w-30 dark:bg-gray-700 dark:divide-gray-600`}>
                                <div className="text-sm text-gray-900 dark:text-white w-max flex flex-col text-center">
                                    {navBarPages.map(([title, url], i) => (
                                        <Link key={i} href={url} className={`${i == 0 ? "rounded-t-lg" : (i == navBarPages.length - 1 ? "rounded-b-lg" : "")} px-7 py-2 text-black font-semibold hover:bg-slate-200`} style={{textDecoration: 'none'}}>
                                            {title}
                                        </Link>
                                    ))}
                                </div>
                    ...
                    <nav className="hidden mlg:flex mlg:order-1" id="mobile-menu-2">
                        {navBarPages.map(([title, url], i) => (
                            <Link key={i} href={url} className="rounded-lg px-7 py-2 text-black font-semibold group transition duration-300 hover:scale-110" style={{textDecoration: 'none'}}>
                                {title}
