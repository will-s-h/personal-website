        ...
        <ul className="grid gap-4 grid-cols-1 mlg:grid-cols-3">
            {allPostsData.map(({ id, title, description, featured, draft }, i) => 
                ...
                <Link key={i} href={`/posts/${id}`}>
                    ...
                    <span className="block truncate text-xl pb-2 font-sans font-extrabold text-black dark:text-white group-hover:text-yellow">{title}</span>
                    <span className="font-sans text-sm text-black dark:text-white font-semibold group-hover:text-yellow">{description}</span>
                    </div>
                </Link>
                )
            )}
