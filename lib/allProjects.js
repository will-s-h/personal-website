        ...
        <ul className="grid gap-4 grid-cols-1 mlg:grid-cols-3">
            {allProjectsData.map(({ id, title, description, featured, draft }, i) => 
                ...
                <Link key={i} href={`/projects/${id}`}>
                    ...
                    <span className="block truncate text-xl pb-2 font-sans font-extrabold text-black group-hover:text-red">{title}</span>
                    <span className="font-sans text-sm font-semibold text-black group-hover:text-red">{description}</span>
                    </div>
                </Link>
                )
            )}
